...
pub trait Provider: Clone + Sync + Send {
    ...
    async fn fetch_current_block_number(&self) -> u64 {
    ...
    async fn fetch_logs(&self, filters: &Vec<Filter>) -> Vec<Log> {
        let mut maybe_logs = None;
        let mut retries_so_far = 0;

        while maybe_logs.is_none() {
            match try_join_all(filters.iter().map(|f| self.get_logs(&f.value))).await {
                Ok(logs_per_filter) => {
    ...
    async fn fetch_blocks_by_number(&self, logs: &Vec<Log>) -> HashMap<U64, Block<TxHash>> {
        let mut maybe_blocks_by_number = None;
        let mut retries_so_far = 0;

        while maybe_blocks_by_number.is_none() {
            match self.get_blocks_by_number(logs).await {
