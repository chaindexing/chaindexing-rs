...
use events::Event;
use repo::{ExecutesWithRawQuery, LoadsDataWithRawQuery, Repo};
...
pub async fn backtrack_states<'a, C: ExecutesWithRawQuery>(
...
pub fn get_all_table_names(state_migrations: &[Arc<dyn ContractStateMigrations>]) -> Vec<String> {
    state_migrations
        .iter()
