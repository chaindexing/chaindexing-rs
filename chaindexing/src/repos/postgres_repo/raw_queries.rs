...
use crate::{
    ExecutesWithRawQuery, HasRawQueryClient, LoadsDataWithRawQuery, PostgresRepo, RepoError,
};
...
impl HasRawQueryClient for PostgresRepo {
    ...
    async fn get_client(&self) -> Result<Self::RawQueryClient, RepoError> {
        tls::connect_raw_client(&self.url, self.tls_config())
            .await
    ...
    async fn get_txn_client<'a>(
        ...
        client
            .transaction()
            .await
            .map_err(|error| RepoError::Transaction(error.to_string()))
    }
}
...
impl ExecutesWithRawQuery for PostgresRepo {
    async fn execute(client: &Self::RawQueryClient, query: &str) -> Result<(), RepoError> {
        client
            .execute(query, &[] as &[&(dyn ToSql + Sync)])
            .await
            .map(|_| ())
            .map_err(|error| RepoError::Query(error.to_string()))
    }
    ...
    async fn commit_txns<'a>(client: Self::RawQueryTxnClient<'a>) -> Result<(), RepoError> {
        client.commit().await.map_err(|error| RepoError::Transaction(error.to_string()))
    }
...
impl LoadsDataWithRawQuery for PostgresRepo {
    async fn create_and_load_new_node(client: &Self::RawQueryClient) -> Result<Node, RepoError> {
        ...
        .await?
        .ok_or_else(|| RepoError::Cardinality("node insert returned no rows".to_string()))
    }
    ...
    async fn load_data<Data: Send + DeserializeOwned>(
        ...
        if data_list.len() > 1 {
            return Err(RepoError::Cardinality(format!(
    ...
    async fn load_data_list<Data: Send + DeserializeOwned>(
        ...
        if json_aggregate.is_object() || json_aggregate.is_array() {
            serde_json::from_value(json_aggregate)
                .map_err(|error| RepoError::Decode(error.to_string()))
        } else {
            Ok(vec![])
        }
    }
...
async fn get_json_aggregate(
    client: &PostgresRepoClient,
    query: &str,
) -> Result<serde_json::Value, RepoError> {
    let rows = client
        .query(json_aggregate_query(query).as_str(), &[])
        .await
        .map_err(|error| RepoError::Query(error.to_string()))?;

    rows.first()
        .map(|row| row.get(0))
        .ok_or_else(|| RepoError::Cardinality("aggregate query returned no rows".to_string()))
}
