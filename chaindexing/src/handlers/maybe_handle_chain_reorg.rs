...
use crate::{ChaindexingRepoClient, ExecutesWithRawQuery, HasRawQueryClient, RepoError};

pub async fn run(
    repo_client: &mut ChaindexingRepoClient,
    table_names: &[String],
) -> Result<(), RepoError> {
    ...
    if !reorged_blocks.is_empty() {
        ...
        {
            states::backtrack_states(table_names, *chain_id, *block_number, &repo_txn_client)
                .await?;
